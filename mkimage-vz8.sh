#!/usr/bin/env bash
#
# Create a base Virtuozzo8 Docker image.
#
# This script is useful on systems with yum installed (e.g., building
# a Vz image on Vz itself).
#
...
install_groups=()
install_packages=('virtuozzo-release')
version=
...
[vz-build]
name=VirtuozzoBuild
baseurl=http://repo.virtuozzo.com/vz/releases/${BUILD}/x86_64/os/
enabled=1
gpgcheck=0
priority=50
...
cp vzlinux8.repo "$target"/etc/yum.repos.d/
...
if [[ -n "$install_groups" ]];
then
    yum -c "$yum_config" --disablerepo=* --enablerepo=vz-build --enablerepo=vzl8 --installroot="$target" --releasever=/ --setopt=tsflags=nodocs \
...
rm -f "$target"/etc/yum.repos.d/vz-build.repo
